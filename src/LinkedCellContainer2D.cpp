...
void LinkedCellContainer2D::deleteUpperHalo() {
    for(int i = 0; (int) i < linkedCells.size(); ++i) {
...
void LinkedCellContainer2D::initialiseCells(std::vector<Particle>& particleVector) {
    for (size_t i = 0; i < particleVector.size(); ++i)
    {
        ...
        int Xindex = particleToMap.getX().at(0)/cellSize.at(0) + 1; // the x - index of the particle, after halo cells
        int Yindex = particleToMap.getX().at(1)/cellSize.at(1) + 1; // the y-index of the particle, after halo cells
